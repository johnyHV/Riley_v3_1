...
mux4067 mux(mux_control_pin,mux_initial,mux_output);            // Vytvorenie objektu MUX
...
static motor_wheel wheel = {3.3,2048.0,27.7735,8400.0};                          // specifikacia kolesa a motora


// IR senzory
//mux,critical,komparator,analog
IR_proximity proximity = {false,{2,3},{41,42},{2,3}};           // pripojenie proximitno reflexnych senzorov
const uint8_t sharp_f = 4;                                      // pripojenie Sharp predneho senzoru
const uint8_t sharp_b = 5;                                      // pripojenie Sharp zadneho senzoru
...
void setup() {
	...
	motor_initial(mux,&motor_f);                            //inicializacia predneho motora
	motor_initial(mux,&motor_b);                            //inicializacina zadneho motora
	...
	if (Serial.available() > 0) {
		...
		if ( incomingByte == 119) {							// w - dopredu
			...
			scan(); }
                    ...
                    Serial.println(senzor_check(motor_l,motor_r,wheel,uz_f,uz_b,sharp_f,sharp_b,proximity,mux,FRONT));
                }
                    ...
                    Serial.println(stepst);
                    Serial.println(motor_l.full_step);
                    Serial.println(motor_l.number_step);
